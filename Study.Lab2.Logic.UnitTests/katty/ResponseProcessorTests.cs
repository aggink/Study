...
public class ResponseProcessorTests
{
    ...
    public void IsSuccessResponse_ВозвращаетTrue_ПриКорректномJSON()
    {
        ...
        // Act
        bool result = _responseProcessor.IsSuccessResponse(response);
    ...
    public void ProcessResponse_ФорматируетJSON_ПриКорректномJSON()
    {
        ...
        string expected = JsonSerializer.Serialize(jsonElement, new JsonSerializerOptions { WriteIndented = true });

        // Act
        string result = _responseProcessor.ProcessResponse(response);
